...
glm::vec3 Camera::GetVelocity()
{
	glm::vec3 forwardVector = 
		glm::vec3(camTarget.x - camPos.x, 0.0f, camTarget.z - camPos.z);
...
void Camera::MoveForward(float units, CollisionVolumeCylinder &camVolume)
{
	glm::vec3 forwardVector = glm::vec3(camTarget.x - camPos.x, 0.0f, camTarget.z - camPos.z);
	glm::normalize(forwardVector);
	
	camVolume.SetPosition(camPos);		
	camVolume.SetVelocity(forwardVector);

	if(camVolume.CheckCollisionAABB(glm::vec3(12.0f, 0.5f, -1.0f), glm::vec3(28.0f, 0.5f, -21.0f)) == false)
	{			
		camTarget += units * forwardVector;
	}
	camPos = ResolveCamPosition();
}
void Camera::MoveBackward(float units, CollisionVolumeCylinder camVolume)
{
...
void Camera::MoveLeft(float units, CollisionVolumeCylinder camVolume)
{
	glm::vec3 forwardVector(camTarget.x - camPos.x, 0.0f, camTarget.z - camPos.z);
	...
	camVolume.SetPosition(camPos);		
	camVolume.SetVelocity(-rightVector);
...
void Camera::MoveRight(float units, CollisionVolumeCylinder camVolume)
{
	glm::vec3 forwardVector(camTarget.x - camPos.x, 0.0f, camTarget.z - camPos.z);
	glm::vec3 rightVector = glm::cross(forwardVector, upVector);
	glm::normalize(rightVector);
